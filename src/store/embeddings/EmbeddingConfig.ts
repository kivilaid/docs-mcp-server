 ...
 * Built dynamically from KNOWN_MODEL_DIMENSIONS to ensure consistency.
 */
let LOWERCASE_MODEL_LOOKUP: Record<string, number> | null = null;

/**
 * Initialize the lowercase lookup map if not already created.
 */
function initializeLowercaseLookup(): void {
  if (LOWERCASE_MODEL_LOOKUP === null) {
    LOWERCASE_MODEL_LOOKUP = {};
    for (const [model, dimensions] of Object.entries(KNOWN_MODEL_DIMENSIONS)) {
      LOWERCASE_MODEL_LOOKUP[model.toLowerCase()] = dimensions;
    }
  }
}
 ...
 */
export function parseEmbeddingConfig(modelSpec?: string): EmbeddingModelConfig {
  const spec =
  ...
  initializeLowercaseLookup();
  const dimensions = LOWERCASE_MODEL_LOOKUP?.[model.toLowerCase()] || null;
 ...
 */
export function getKnownModelDimensions(model: string): number | null {
  initializeLowercaseLookup();
  return LOWERCASE_MODEL_LOOKUP?.[model.toLowerCase()] || null;
}
 ...
 */
export function setKnownModelDimensions(model: string, dimensions: number): void {
  KNOWN_MODEL_DIMENSIONS[model] = dimensions;

  // Update lowercase lookup map
  initializeLowercaseLookup();
  if (LOWERCASE_MODEL_LOOKUP) {
    LOWERCASE_MODEL_LOOKUP[model.toLowerCase()] = dimensions;
  }
}
